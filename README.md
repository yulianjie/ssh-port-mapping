...
- SSH 异常退出或启动失败后自动重连，采用有上限的指数退避
- 自动重连全部失败后强制发送 Windows 系统通知
- 关闭窗口后继续在系统托盘运行
- 实时显示 SSH 错误和异常退出状态
- 配置仅保存在本机 JSON 文件中
...
新建隧道和从旧版配置载入的隧道默认启用自动重连：总共重试 10 次，初始间隔 5 秒。
等待时间采用指数退避，每次翻倍并将单次等待限制在 5 分钟；默认序列为
`5、10、20、40、80、160、300、300、300、300` 秒。可以在隧道编辑页的
**断线恢复** 区域单独修改初始间隔、次数或关闭自动重连。SSH 异常退出以及无法启动
`ssh.exe` 都会进入同一套重试流程；用户手动停止隧道、删除隧道或退出 PortWeave
会立即取消待执行的重试。

如果一次重连后的连接连续运行至少 30 秒，本轮失败计数会视为已经恢复；以后再次断线
时重新获得完整的重试次数。等待期间，隧道卡片会显示下一次重试的倒计时和当前次数，
活动日志会保留每次调度、启动与最终失败结果。

所有重试均失败后，PortWeave 会在界面顶部和隧道卡片中保留错误，并强制请求发送
Windows 系统通知；此行为没有关闭开关，即使主窗口已经隐藏到托盘也会执行。可以在
**设置 → 应用行为** 中点击 **发送测试通知** 检查当前 Windows 通知环境。Windows
仍可能根据用户的勿扰或系统通知策略决定通知的最终呈现方式。
...
开发或集成测试时，可以只为测试进程设置 `PORTWEAVE_CONFIG_DIR`，将 `config.json`
重定向到隔离目录；未设置时始终使用上面的正常用户路径。
